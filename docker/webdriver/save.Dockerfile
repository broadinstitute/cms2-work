...
# Setup packages
USER root
RUN apt-get -m update && DEBIAN_FRONTEND=noninteractive DEBCONF_NONINTERACTIVE_SEEN=true apt-get install -y locales wget unzip curl zip python3 chromium-browser chromium-chromedriver python3 python3-pip python3-selenium
...
# #RUN conda install -c file:///tmp/miniconda/miniconda/conda-bld tar numpy scipy matplotlib pandas pytables h5py psutil tabix selscan=1.3.0a09 selenium && pip3 install chromedriver-binary=2.38 chromedriver-binary-auto && conda clean -a -y
# #RUN conda install tar numpy scipy matplotlib pandas pytables h5py psutil tabix selenium && pip3 install chromedriver-binary-auto && conda clean -a -y
# #COPY selenium-env.txt /tmp
# #RUN conda create -n selenium-env --file /tmp/selenium-env.txt 
# #RUN conda init bash
# #RUN conda clean -a -y

# RUN conda install selenium && pip3 install chromedriver-binary-auto && conda clean -a -y
